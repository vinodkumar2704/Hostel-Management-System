...
<?php
   if (isset($_POST['search'])) {
   	   ...
   	   $query_search = "SELECT * FROM student WHERE s_id like '$search_box%'";
    ...
    <?php
   	   if(mysqli_num_rows($result_search)==0){
            ...
            if(isset($row88['hostel_name'])){
            $hostel_name = $row88['hostel_name'];}
            else{$hostel_name='None';}
            //student name
         ...
         echo '<tr><td colspan="4">No Rows Returned</td></tr>';
      	...
      	while($row1 = mysqli_fetch_assoc($result1)){
            ...
            $student_name = $row1['fname']." ".$row1['lname'];
      		...
      		echo "<tr><td>{$student_name}</td><td>{$row1['s_id']}</td><td>{$row1['phone_no']}</td><td>{$HNM}</td><td>{$room_no}</td></tr>\n";
      	}
      }
    ?>
    </tbody>
