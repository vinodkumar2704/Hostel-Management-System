			...
			<nav class="navbar navbar-expand-lg navbar-light bg-light">
				...
				<div class="collapse navbar-collapse justify-content-center" id="navbarSupportedContent">
					<ul class="navbar-nav ml-auto">
						...
						<li class="nav-item">
							<a class="nav-link active" href="allocate_room.php">Allocate Rooms</a>
						</li>
						<li class="nav-item">
   ...
   if (isset($_POST['search'])) {
    ...
    <?php
   	   if(mysqli_num_rows($result_search)==0){
            ...
            $query7 = "SELECT * FROM student WHERE s_id = '$student_id'";
      ...
      if(mysqli_num_rows($result1)==0){
         echo '<tr><td colspan="4">No Rows Returned</td></tr>';
      	...
      	while($row1 = mysqli_fetch_assoc($result1)){
            ...
            $row7 = mysqli_fetch_assoc($result7);
            $student_name = $row7['fname']." ".$row7['lname'];
            //<td>{<form action=\"allocate_room.php\" method=\"post\"><input type=\"submit\" value=\"Allocate\" name=\"submit\"></form>}</td>
...
<section class="contact py-2">
	<div class="container">
			<div class="mail_grid_w3l"><div class="row">
				<form action="allocate_room.php" method="post">
					 
							<input type="submit" value="Allocate all" name="submit">
					
				</form>   .   .
				<form action="allocate_room.php" method="post">
							<input type="submit" value="Reject all" name="reject">
				</form></div>
			</div>
	</div>
</section>
...
if(isset($_POST['submit'])){
   ...
   while($row1 = mysqli_fetch_assoc($result1)){
     ...
     $query2 = "SELECT * FROM room where h_id = '$hostel_id' and room_no = (SELECT MIN(room_no) FROM room where allocated = '0' and h_id = '$hostel_id')";
     ...
     if($result3){
     	...
     	$query4 = "UPDATE student SET h_id = '$hostel_id',room_id = '$room_id' WHERE s_id = '$student_id'";
     	$result4 = mysqli_query($conn,$query4);
     	if($result4){
     		$query5 = "UPDATE room SET allocated = '1' where room_id = '$room_id' and h_id = '$hostel_id'";
...
if(isset($_POST['reject'])){
   ...
   while($row1 = mysqli_fetch_assoc($result1)){
         //find the minimum room number
	   	$student_id = $row1['Student_id'];
	    $query3 ="DELETE FROM Application WHERE Student_id = '$student_id'"; 
	    $result3 = mysqli_query($conn,$query3);
	     /*echo "<script type='text/javascript'>alert('<?php echo $result3; ?>')</script>";*/
